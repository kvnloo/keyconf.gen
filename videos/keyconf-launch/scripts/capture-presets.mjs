...
try {
  ...
  for (const [id, name] of [
    ...
    await p
      ...
      .click();
    await ready();
    await p.screenshot({ path: `assets/stills/${id}.png` });
    console.log(`Captured ${name}`);
  }
  await p
    .getByRole('button', { name: 'Preview Forest Line', exact: true })
    .click();
  await ready();
  await p
    ...
    .click();
  await ready();
  await p.getByRole('link', { name: 'Sound', exact: true }).click();
  await ready();
  await p
  ...
  await p.screenshot({ path: 'assets/stills/sound.png' });
  console.log('Captured enabled Sound Lab');
} finally {
  await b.close();
}
