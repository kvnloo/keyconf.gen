...
const context = await browser.newContext({
  ...
  recordVideo: { dir: 'capture/typing', size: { width: 1536, height: 1200 } },
});
...
try {
  ...
  await page
    ...
    .click();
  await page
    .getByRole('button', { name: 'Start typing test', exact: true })
    .click();
  ...
  if (await frame.locator('#result').isVisible())
    throw new Error('Typing take unexpectedly reached results');
  end = (Date.now() - origin) / 1000;
  await writeFile(
    'capture/typing-events.json',
    JSON.stringify({ start, end, events }, null, 2),
  );
  console.log('End', end);
} catch (error) {
  console.error(error);
  await page.screenshot({ path: 'capture/typing-failure.png' });
  throw error;
} finally {
  const video = page.video();
  await context.close();
  await video.saveAs('assets/footage/typing-take.webm');
  await browser.close();
}
