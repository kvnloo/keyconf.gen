...
await p.goto('http://localhost:3344/renders/stage-preview.html', {
...
for (const t of [...proofTimes, ...proofTimes.toReversed()]) {
  ...
  const hash = createHash('sha256')
    .update(await p.screenshot())
    .digest('hex');
  if (baseline.has(t) && baseline.get(t) !== hash)
    throw new Error(`Non-deterministic image at ${t}s`);
  baseline.set(t, hash);
}
await writeFile(
  'snapshots/seek-proof.json',
  JSON.stringify({ ok: true, poses: Object.fromEntries(baseline) }, null, 2),
);
await b.close();
