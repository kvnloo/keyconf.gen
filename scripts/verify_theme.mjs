...
assert.ok(
  page.includes('featured'),
  'Landing featured selection must drive theme',
);
assert.ok(page.includes('applyPaletteTheme'), 'Theme applies palette');

const globalsCss = readFileSync('app/globals.css', 'utf8');
assert.ok(
  !globalsCss.includes('background: #222c26'),
  'Hardcoded #222c26 background must be removed',
);
assert.ok(
...
assert.ok(
  data.boards && data.boards.length >= 9,
  'Premium keyboards data must have 9+ boards',
);
