...
The isolated account fixture now renders `FavoritesPanel` and a candidate `FavoriteButton`, backed by real favorites handlers. `npm run verify:favorites` checks add/remove, withdrawal redaction, repeat mutations, pagination, account isolation and mobile accessibility. The panel and button are not wired into public navigation or public build pages. Hosted authentication, shared favorite state between surfaces and real cross-device verification remain activation work.
