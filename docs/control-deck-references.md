...
Original Blender assets are generated by `scripts/build_control_decks.py`; `scripts/render_control_decks.py` renders thumbnails from those GLBs. The Grok study has 11 cap groups and 46 mesh batches. The Codex study has 12 cap groups, with two switch bodies under its wide key, and 42 mesh batches. Corners, cap profiles and interior proportions remain illustrative. The shader uses the user's chosen colors; rendered thumbnails capture the initial asset materials.
