...
| COMMUNITY-2 | Saved and favorite builds | Account-backed private drafts and favorites survive devices; favorites reference real published builds; no publication caused by saving | In progress: private build storage shipped; favorite storage implemented with owner isolation and withdrawal redaction; authenticated UI and cross-device verification pending |
| COMMUNITY-3 | Creator build pages | Creator attribution, immutable published revision, source-backed parts and honest sound/geometry scope; recipient can experience it without changing existing local work | In progress: immutable publication storage and dynamic attributed build pages shipped; frozen evidence and retired-part fallback verified; sign-in and creator publishing controls remain pending |
| COMMUNITY-4 | Client proposals | Creator sends an unlisted preconfigured preview; client can leave feedback tied to the viewed revision and create a separate customization; distinguish a proposal from an accepted commission | In progress: internal immutable proposal creation, token-hash preview reads and owner closure implemented; rotation storage and owner listing implemented; response storage implemented; private invitation read route and isolated client preview published in Sites version 27; authenticated creation/submission UI pending |
...
| COMMUNITY-6 | Minimal discovery | One prominent search/command entry and a few primary destinations; contextual actions expose favorite, share and customize; no fake AI answers or empty dashboard modules | In progress: unified part/accessory and workspace search implemented; community discovery and favorites await accounts |
...
| COMMUNITY-8 | Optional music | Off by default; independent music volume/mute; keyboard recordings and sound-reference videos take priority and mute music, with gentle restoration only if the user still wants music; visibility, loading/error and autoplay rules tested | Shipped on nightly: opt-in licensed music, independent volume, keyboard/reference priority, browser and unit checks |
...
| PART-3 | Dials and encoders | Distinguish a replacement knob from an encoder; placement, shaft/mount, PCB and firmware requirements affect compatibility; selected parts survive save/share/undo | In progress: Q1 Max ANSI stock knob geometry and manufacturer-supported replacement-cap slot shipped; other hosts and added encoder electronics remain |
| PART-4 | Screens and extra controls | Distinguish integrated modules from external displays, buttons and macropads; mounting, connectors, power and firmware support are sourced or unknown; no arbitrary snap-on fit | In progress: module planning, unmounted OLED/button/encoder tray studies and external macropad preview implemented; embedded host integration remains |
...
| FLOW-3    | Share and revisit                 | A self-contained link reconstructs appearance, selected parts, and audio preference on a clean device; downloads retain evidence                                                           | Portable links open isolated previews; explicit copy, Undo, cross-tab isolation, file restore and selected imports verified                              |
...
| SHIP-1    | Reproducible public release       | Passing types/tests/builds, committed source, successful GitHub CI and public deployment from that source                                                                                  | Latest verified release: d9ba244, Sites version 27; GitHub run 34070756706 passed; live invitation privacy and rejection checks passed                |
