...
| COMMUNITY-4 | Client proposals | Creator sends an unlisted preconfigured preview; client can leave feedback tied to the viewed revision and create a separate customization; distinguish a proposal from an accepted commission | In progress: internal immutable proposal creation, token-hash preview reads and owner closure implemented; rotation storage and owner listing implemented; response storage implemented; routes and authenticated UI pending |
