...
The Pages environment permits deployments from main, dev and nightly. Pull requests never deploy. Publishing jobs share one concurrency group across all three branches. A failing check retains the previous publication. GitHub can replace an older pending concurrency job with a newer one; rerun that branch's successful workflow if its preview has not advanced. No overlapping jobs can overwrite another channel's output.
