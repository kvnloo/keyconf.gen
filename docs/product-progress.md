...
## September 9, 2026: CAD twin pipeline

Locked CAD twin as the geometry bar. A product is a CAD twin only when case, plate and PCB are cad-derived or measured from licensed STEP/STL or published millimetre solids. Photos cannot promote grade. Featured and premium copy now use that gate; HATSU, CYBERBOARD R2 and Q1 Max remain studies (Q1 knob is dimension-verified 16×14 mm). Added CadQuery STEP-to-GLB generator `scripts/cad_twin.py` and per-part revisions in `docs/reference-assets/cad-revisions.json`. No manufacturer CAD is in the repository.

## CAD detect-and-display beta

The studio can now overlay a licensed mesh onto a study model, and the revision manifest is the only gate. A part displays a mesh only when it lists `scene.glb` under `public/models/cad/` with a 64-character hash and its grade is `dimension-verified`, `cad-derived` or `measured`; the two CAD grades also require a licence and a source hash. Illustrative and layout-verified parts never display, even with a mesh listed, and a path that leaves the CAD directory is refused. There is no runtime flag: listing a file is the only way to turn an overlay on.
...
## First CAD part on screen

The detect-and-display path now has something to display. CadQuery 2.8.0 with OpenCascade 7.9.3 runs in an isolated virtualenv outside the repository, and `scripts/cad_twin.py knob` produces the Q1 encoder knob as a parametric solid from Keychron's published outer diameter and height. The generator writes `public/models/cad/keychron-aluminum-knob.step` and its `.glb`, prints the GLB hash, and that hash is what the revision lists. Both files are ours, built from published millimetres. No manufacturer CAD is in the repository, and the knob is still `dimension-verified`, so no board became a twin: the Q1 case, plate and PCB remain illustrative and layout-verified, and `data-cad-twin` stays `0`.

Putting a generated solid on screen for the first time exposed two things the empty beta could not. CadQuery models Z-up in millimetres while glTF is Y-up, so the exported knob lay on its side; `write_glb` now turns the mesh -90 degrees about X, which preserves winding so the faces stay outward. Separately, the solid's origin sits at its base while the study cylinder is centred on its own, so copying the study's position buried the part half its height out of place. The overlay now seats a part by the volume it occupies rather than trusting whatever origin its author left, which is the rule licensed CAD will need too.

Both fixes were measured rather than eyeballed, against a render of the same board with the knob delisted. Rebuilding the solid at 42 mm instead of 14 mm raised the silhouette's top edge by 18 pixels and nearly doubled the changed pixels while its width stayed flat, so the modelled height maps to the screen's vertical axis and the knob stands upright. For seating, the CAD knob and the study share identical published dimensions, so the correct placement is the one that best reproduces the study: with centring the render differs from the study by 968 pixels, and with centring disabled by 1413. The live board reports `data-cad-parts="1"`, `data-cad-twin="0"` and 81 switches with no page errors and no failed model request, and the served GLB hashes to the value the manifest lists.

Two limits are worth recording. The generated GLB carries positions and indices only, so the loader shades it flat with a default material instead of the study's metallic finish; the geometry is right and the appearance is not yet. And this is one part on one board, which is the point of the gate rather than a shortcoming: the first licensed plate is the next candidate, and until case, plate and PCB are licensed or measured, nothing here is a CAD twin.

## The CAD knob takes the finish it replaces

The previous entry recorded one open limit: the generated GLB carried positions and indices only, so the loader lit it with a default white material and the knob read as a pale silhouette beside the study it replaced. That is now closed from both ends.

`scripts/cad_twin.py` emits a NORMAL attribute alongside POSITION, averaging the facet normals meeting at each vertex. OpenCascade tessellates face by face, so a point on a sharp edge is emitted once per face and the averaging never crosses that edge: the knob's wall stays smooth, its caps stay flat and its rim stays sharp. Because that is a property of the tessellator rather than a guarantee, the export refuses a vertex whose facets disagree by more than 60 degrees instead of quietly rounding the edge off. `applyCadOverlay` then dresses the loaded part in the study's own material and disposes the one the loader supplied, so a CAD part contributes shape while the palette and appearance system keep owning finish.

The evidence is a render of the same Q1 board in three states, compared against the study with the knob delisted. The old positions-only knob differed from the study by 968 pixels; the new one differs by 717, and differs from its own previous render by 941 — the appearance changed almost everywhere the knob is drawn. Colour settles it. In the 90x95 crop where those two renders disagree, the study reads R127 G128 B112 with a standard deviation of 62, the old CAD knob R137 G138 B122 at 68, and the new CAD knob R126 G128 B111 at 62. The old part sat about ten levels brighter on every channel, which is the flat white default; the new part matches the study within a single level per channel with an identical spread. The 717 pixels that still differ are the tessellated solid against the study's 64-segment cylinder, which is geometry rather than finish.

The GLB is internally consistent: its 1012 normals are all unit length, its extent is still 16 x 14 mm with the tall axis on Y, and its hash matches the manifest and the asset the server actually delivers. 236 tests, type checking, lint and formatting pass, and the live board still reports `data-cad-parts="1"`, `data-cad-twin="0"` and 81 switches with no page errors. Nothing here promotes a grade: the knob remains dimension-verified and no board is a CAD twin.

One consequence is worth stating plainly. Because a CAD part wears the study's material, a file carrying several distinct materials would be flattened to one. That is the right trade while parts are single-material solids built from published millimetres, and it is the thing to revisit when a licensed multi-material assembly arrives.

## A drop says whose sale it is

A creator drop already carried the parts COMMUNITY-5 asks for: a deliberate publication distinct from a build, a creator-written availability line, an HTTPS-only enquiry link, and a discovery filter that separates drops from builds. `verify:creator-review` drives that whole path in a real browser, from checking "Creator drop" through to the stored metadata. The row was still marked Pending, which understated it.

What was actually missing sat at the other end, on the page a visitor reads. The availability line was rendered as a bare paragraph inside our own chrome, so nothing distinguished the creator's claim about their own sale from stock information we were asserting ourselves. That is the part of the requirement that forbids invented stock, checkout or affiliation, and absence of a checkout button is not the same as saying so. The page now labels the line as the creator's, and states that Keyconf holds no stock, takes no payment and does not represent the seller, adding that the link leaves the site when there is a link to leave by. `verify:publication` reads that text out of the rendered page, so the disclosure is asserted where a visitor would see it rather than trusted to stay.
