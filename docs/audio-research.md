...
Build broad switch coverage from existing recordings. Import reusable sound packs and index creator-hosted typing tests, preserving attribution and known build details. Personally recording hundreds of keyboards is not a prerequisite. Measurement-calibrated models remain a later option for predicting modifications.

The app now bundles seven recorded switch presets and indexes 266 unique Click and Thock test videos. Sample-specific MIT notices were verified in the [Keyboard Sounds packs](https://github.com/nathan-fiscaletti/keyboardsounds/tree/6785c7ac17b9f4db1f3d7f3d1d87a23067944517/keyboardsounds/profiles). The 78 original MP3 files retain their notices and hashes. Videos use the original YouTube player. The indexed collection is substantial but does not cover every switch.
...
The original engine used random noise and a falling sine tone. Recorded presets now use predecoded source samples, separate releases, repeated sample selection, modifier mappings, and audio-clock demo scheduling. The synthetic study remains optional. No measured assembly simulation or binaural source capture has been added, and perceptual fidelity has not been independently validated.
...
| [kbsim](https://github.com/tplai/kbsim) | Browser simulator; inspected assets include 151 MP3 files with press/release, row, and modifier groupings. Seven downstream packs provide explicit audio-specific MIT notices and are now imported. | Full build and capture information remains incomplete. These are recorded switch references, not validated predictions of a selected assembly. |
...
These are source and asset-structure checks, not headphone auditions or independent measurements of playback quality. The seven imported packs were decoded successfully and their bytes checked against the pinned manifest.
...
1. **Existing recording release.** Import licensed per-switch samples and index original creator videos. The first implementation includes seven playable packs, separate down/up playback, variation, available modifier mappings, audio-clock demos, source notices, and 266 searchable video references. Expand through additional source adapters and reusable packs. Keep missing coverage visible.
